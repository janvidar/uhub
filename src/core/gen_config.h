...
struct hub_config
{
	...
	int   metrics_enable;                  /*<<< Enable the Prometheus metrics endpoint (default: 0) */
	char* metrics_token;                   /*<<< Bearer token required to read the metrics endpoint (default: "") */
	char* metrics_path;                    /*<<< Request path that serves the metrics document (default: "/metrics") */
	int   workers;                         /*<<< Run one logical hub as this many worker processes (default: 1) */
	char* worker_socket_dir;               /*<<< Directory for inter-worker link sockets (default: "/tmp") */
	int   server_reuseport;                /*<<< Allow several hub processes to share the server port (default: 0) */
	...
	int   max_connections_per_ip;          /*<<< Maximum concurrent connections allowed per IP address (default: 0) */
	int   keepalive_interval;              /*<<< Seconds of write silence before the hub pokes an idle client (default: 120) */
	...
	char* hub_website;                     /*<<< Hub website address advertised to hub lists (WS). (default: "") */
	char* hub_network;                     /*<<< Hub network name advertised to hub lists (NE). (default: "") */
	char* hub_owner;                       /*<<< Hub owner name advertised to hub lists (OW). (default: "") */
	char* reg_server_url;                  /*<<< ADC registration server URL to announce this hub to at startup. (default: "") */
	...
	int   limit_max_slots;                 /*<<< Limit maximum number of upload slots open per user (default: 0) */
	int   limit_min_search;                /*<<< Limit minimum search term length (default: 0) */
	...
	char* tls_ciphersuite;                 /*<<< List of TLS 1.2 (and earlier) ciphers to use (default: "ECDH+AESGCM:DH+AESGCM:ECDH+AES256:DH+AES256:ECDH+AES128:DH+AES:ECDH+3DES:DH+3DES:RSA+AESGCM:RSA+AES:RSA+3DES:!aNULL:!MD5:!DSS") */
	char* tls_ciphersuites;                /*<<< List of TLS 1.3 cipher suites to use (default: "TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:TLS_AES_128_GCM_SHA256") */
	char* tls_version;                     /*<<< Specify minimum TLS version supported. (default: "1.2") */
	int   adcs_translate;                  /*<<< Translate between the two spellings of the ADCS extension (default: 0) */
	char* file_acl;                        /*<<< File containing access control lists (bans and denied nicks) (default: "") */
	int   bbs_enable;                      /*<<< Enable bulletin boards (the BBS0 extension) (default: 0) */
	char* file_bbs_boards;                 /*<<< File defining the bulletin boards this hub serves (default: "") */
	char* file_bbs_index;                  /*<<< SQLite database holding the bulletin board index (default: "") */
	int   bbs_max_subscriptions;           /*<<< Maximum number of boards one session may subscribe to at once (default: 8) */
	int   bbs_post_interval;               /*<<< Minimum number of seconds between two posts from one session (default: 60) */
	int   bbs_max_posts_per_board;         /*<<< Maximum number of index entries retained per board (default: 5000) */
	char* nat_override;                    /*<<< Comma separated list of IP ranges allowed to advertise their own address (default: "") */
	...
	char* msg_search_too_short;            /*<<< "Search term is too short." */
	char* msg_user_flood_update;           /*<<< "Update flood detected." */
	char* msg_user_flood_extras;           /*<<< "Flood detected." */
	char* msg_chat_is_privileged;          /*<<< "Chat is reserved for operators and above." */
