 ...
 * Created 2026-08-19 20:43, by config.py
 */
...
void config_defaults(struct hub_config* config)
{
	...
	config->workers = 1;
	config->worker_socket_dir = hub_strdup("/tmp");
	config->server_reuseport = 0;
	...
	config->adcs_translate = 0;
	config->file_acl = hub_strdup("");
	config->bbs_enable = 0;
	config->file_bbs_boards = hub_strdup("");
	config->file_bbs_index = hub_strdup("");
	config->bbs_max_subscriptions = 8;
	config->bbs_post_interval = 60;
	config->bbs_max_posts_per_board = 5000;
	config->nat_override = hub_strdup("");
...
static int apply_config(struct hub_config* config, char* key, char* data, int line_count)
{
	...
	if (!strcmp(key, "workers"))
	{
		min = 0;
		max = 256;
		if (!apply_integer(key, data, &config->workers, &min, &max))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "worker_socket_dir"))
	{
		if (!apply_string(key, data, &config->worker_socket_dir, NULL))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "server_reuseport"))
	{
		if (!apply_boolean(key, data, &config->server_reuseport))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "link_socket"))
	{
		if (!apply_string(key, data, &config->link_socket, NULL))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "max_connections_per_ip"))
	{
		min = 0;
		max = 1048576;
		if (!apply_integer(key, data, &config->max_connections_per_ip, &min, &max))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "keepalive_interval"))
	{
		min = 0;
		max = 3600;
		if (!apply_integer(key, data, &config->keepalive_interval, &min, &max))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "adcs_translate"))
	{
		if (!apply_boolean(key, data, &config->adcs_translate))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "bbs_enable"))
	{
		if (!apply_boolean(key, data, &config->bbs_enable))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "file_bbs_boards"))
	{
		if (!apply_string(key, data, &config->file_bbs_boards, NULL))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "file_bbs_index"))
	{
		if (!apply_string(key, data, &config->file_bbs_index, NULL))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "bbs_max_subscriptions"))
	{
		min = 1;
		max = 64;
		if (!apply_integer(key, data, &config->bbs_max_subscriptions, &min, &max))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "bbs_post_interval"))
	{
		min = 0;
		max = 86400;
		if (!apply_integer(key, data, &config->bbs_post_interval, &min, &max))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "bbs_max_posts_per_board"))
	{
		min = 0;
		max = 1000000;
		if (!apply_integer(key, data, &config->bbs_max_posts_per_board, &min, &max))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	if (!strcmp(key, "nat_override"))
	{
		if (!apply_string(key, data, &config->nat_override, NULL))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
...
void free_config(struct hub_config* config)
{
	...
	hub_free(config->worker_socket_dir);

	hub_free(config->link_peer);

	hub_free(config->link_secret);

	hub_free(config->link_socket);
	...
	hub_free(config->file_bbs_boards);

	hub_free(config->file_bbs_index);

	hub_free(config->nat_override);
...
void dump_config(struct hub_config* config, int ignore_defaults)
{
	...
	if (!ignore_defaults || config->workers != 1)
		fprintf(stdout, "workers = %d\n", config->workers);

	if (!ignore_defaults || strcmp(config->worker_socket_dir, "/tmp") != 0)
		fprintf(stdout, "worker_socket_dir = \"%s\"\n", config->worker_socket_dir);

	if (!ignore_defaults || config->server_reuseport != 0)
		fprintf(stdout, "server_reuseport = %s\n", config->server_reuseport ? "yes" : "no");
	...
	if (!ignore_defaults || config->max_connections_per_ip != 0)
		fprintf(stdout, "max_connections_per_ip = %d\n", config->max_connections_per_ip);

	if (!ignore_defaults || config->keepalive_interval != 120)
		fprintf(stdout, "keepalive_interval = %d\n", config->keepalive_interval);
	...
	if (!ignore_defaults || config->adcs_translate != 0)
		fprintf(stdout, "adcs_translate = %s\n", config->adcs_translate ? "yes" : "no");

	if (!ignore_defaults || strcmp(config->file_acl, "") != 0)
		fprintf(stdout, "file_acl = \"%s\"\n", config->file_acl);

	if (!ignore_defaults || config->bbs_enable != 0)
		fprintf(stdout, "bbs_enable = %s\n", config->bbs_enable ? "yes" : "no");

	if (!ignore_defaults || strcmp(config->file_bbs_boards, "") != 0)
		fprintf(stdout, "file_bbs_boards = \"%s\"\n", config->file_bbs_boards);

	if (!ignore_defaults || strcmp(config->file_bbs_index, "") != 0)
		fprintf(stdout, "file_bbs_index = \"%s\"\n", config->file_bbs_index);

	if (!ignore_defaults || config->bbs_max_subscriptions != 8)
		fprintf(stdout, "bbs_max_subscriptions = %d\n", config->bbs_max_subscriptions);

	if (!ignore_defaults || config->bbs_post_interval != 60)
		fprintf(stdout, "bbs_post_interval = %d\n", config->bbs_post_interval);

	if (!ignore_defaults || config->bbs_max_posts_per_board != 5000)
		fprintf(stdout, "bbs_max_posts_per_board = %d\n", config->bbs_max_posts_per_board);

	if (!ignore_defaults || strcmp(config->nat_override, "") != 0)
		fprintf(stdout, "nat_override = \"%s\"\n", config->nat_override);
