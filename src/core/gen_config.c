 ...
 * Created 2026-06-19 21:00, by config.py
 */
...
static int apply_config(struct hub_config* config, char* key, char* data, int line_count)
{
	...
	{
		if (!apply_string(key, data, &config->hbri_address4, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->hbri_address6, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->hub_address, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->hub_website, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->hub_network, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->hub_owner, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->reg_server_url, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->msg_search_too_short, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
	...
	{
		if (!apply_string(key, data, &config->msg_chat_is_privileged, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
