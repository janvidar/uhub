...
int hub_handle_message(struct hub_info* hub, struct hub_user* u, const char* line, size_t length)
{
	...
	if (cmd)
	{
		switch (cmd->cmd)
		{
			...
			case ADC_CMD_DPSR:
			case ADC_CMD_FPSR:
				/* PFSR (partial file sharing): PSR lists the chunks a user has
				   verified for a hash. An active requester is answered directly
				   over UDP; a passive one is answered through the hub. This is
				   search traffic generated by the client automatically, so it
				   is accounted against the search flood counter and dropped
			...
			case ADC_CMD_EPSR: // spam
				/* PSR is only defined for the F and U contexts, so a broadcast
				   or echo variant is never legitimate. */
				CHECK_FLOOD(extras, 1);
				break;
